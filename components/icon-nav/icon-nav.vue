<template>
	<view class="flex flex-wrap py-2">
		<view class="flex flex-column align-center justify-center py-1" style="width: 25%;" v-for="(item,index) in list" :key="index" hover-class="bg-light" @click="open(item)">
		...
		methods: {
			open(item){
				...
				switch (item.module){
					...
					default:
					uni.navigateTo({
						url: '/pages/list/list?module='+item.module,
					});
						break;
				}
			}
