		...
		<view class="px-2">
			<view class="comment-list" v-for="(item,index) in comments" :key="index">
				...
				<view class="flex-1">
					...
					<view class="content" @click="openComment({ reply_id:item.id,reply_user:item.user })">{{ item.content }}</view>
					<view class="date">{{ item.created_time | formatTime}}</view>
					...
					<view class="bg-light px-2 mt-2" v-if="item.post_comments && item.post_comments.length > 0">
						<view class="comment-list" v-for="(item2,index2) in item.post_comments" :key="index2">
							...
							<view class="flex-1">
								...
								<view class="content" @click="openComment({ reply_id:item.id,reply_user:item2.user })">{{ item2.content }}</view>
								<view class="date">{{ item2.created_time | formatTime }}</view>		
							</view>
						</view>
					</view>
			...
			<button type="default" size="mini" style="width: 100%;" class="my-2" @click="loadMore" :disabled="loadStatus != 'more'">
