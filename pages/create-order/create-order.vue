<template>
	<view>
		...
		<uni-list>
			<uni-list-item title="优惠券" :showArrow="true" @click="chooseCoupon" clickable>
				<view slot="footer" class="ml-auto font-sm">
					{{ coupon_desc }}
				</view>
			</uni-list-item>
			<uni-list-item title="支付方式">
				<view slot="footer" class="ml-auto text-success">微信支付</view>
			</uni-list-item>
		</uni-list>
		
		<view style="height: 75px;"></view>
		<view class="fixed-bottom p-2 border-top bg-white">
			<main-button v-if="detail.price" @click="submit">立即购买￥{{ price }}</main-button>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getUseableCouponCount(){
				uni.showLoading({
					title: '加载中...',
					mask: false
				});
				this.$api.getUseableCouponCount({
					goods_id:this.id,
